...
/**
 * Standalone program to load a database table from an Excel xlsx file.
 * The class DoUpload can also be used called from the web server application.
 * The first row of the worksheet must contain column labels corresponding
 * to the database columns. The database table must already exist and the
 * data types are determined from the database metadata. The cells must not 
 * contain formulae.  Cells that contain formula will be treated as NULL. If 
 * the sheet contain formulae copy the sheet to a blank sheet using paste-values.
